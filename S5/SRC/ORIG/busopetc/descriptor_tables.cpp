...
// Extern to nullify on start
extern isr_t interrupt_handlers[];

// Zero interrupt items
// GDT & IDT.
...
static void init_gdt()
{
    ...
    gdt_set_gate(3, 0, 0xFFFFFFFF, 0xFA, 0xCF); // user mode CS
    gdt_set_gate(4, 0, 0xFFFFFFFF, 0xF2, 0xCF); // ... DS
...
static void write_tss(s32int num, u16int ss0, u32int esp0)
{
    ...
    // Ensure zeroed descriptor
    memset(&tss_entry, 0, sizeof(tss_entry));

    tss_entry.ss0  = ss0;  // Kernel stack segment
    tss_entry.esp0 = esp0; // ... stack pointer
    
    // cs, ss, ds, es, fs and gs entries in the TSS for Kernel mode Therefore
    // last two bits changes 0x0b and 0x13
    // This TSS can be used to reach ring 3
    tss_entry.cs   = 0x0b;     
